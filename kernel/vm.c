...
int walkcowalloc(pagetable_t ptbl, uint64 va) {
  if (va >= MAXVA) return -1;
  pte_t *pte = walk(ptbl, va, 0);
  if (pte == 0) return -1;
  char *mem;
  uint flags;
...
int checkcowpage(uint64 va, pte_t *pte) {
  return  // va should blow the size of process memory (bytes)
      (*pte & PTE_V) && (*pte & PTE_C);  // pte is COW page
}
